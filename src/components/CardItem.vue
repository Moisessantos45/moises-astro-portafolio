<template>
  <figure
    ...
    <span class="absolute top-2 left-2 bg-cyan-600 text-white text-xs font-bold px-2 py-1 rounded-full z-10">
    ...
    <button id="likeButton" type="button"
      ...
      aria-label="Me gusta" @click="saveLike(proyect)">
      <i :class="[isLiked && 'fa-solid', 'fa-regular fa-thumbs-up icon-like']"></i>
      <span class="text-xs font-bold couter_like">{{
        localProyect.counter_likes
      }}</span>
    </button>

    <img :src="localProyect.image" alt="Hero" :class="[
    ...
    <div class="p-4 flex flex-col h-[calc(100%-9rem)]">
      <h1 class="text-xl font-bold mb-2 text-white hover:text-cyan-400 transition-colors">
        {{ localProyect.title }}
      </h1>
      ...
      <div class="space-y-4">
        <div class="technologies-container min-h-[80px]">
          <ul class="flex flex-wrap gap-1.5">
            <li v-for="(item, index) in proyect.tecnologies" :key="index" :class="[
        ...
        <div class="flex flex-wrap gap-2 justify-center pt-2 border-t border-slate-700">
          ...
          <Link :link="`/Post/${localProyect.id}`" text="Details" icon="external-link" />
...
const saveLike = async (proyect: TypeProject) => {
  try {
    await useProjectsStore().updateProjectLikes(proyect);
