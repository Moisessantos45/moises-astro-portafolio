<template>
  <div>
    ...
    <article v-else v-for="proyect in projects" :key="proyect.id"
      ...
      <figure
        class="flex justify-center items-center shrink-0 mb-6 md:mb-0 md:mr-8 relative group overflow-hidden rounded-xl">
        <img :class="[
        ...
        ]" :src="proyect.image" :alt="proyect.title" />
        <div
      ...
      <div class="flex flex-col flex-grow">
        <div class="flex flex-col md:flex-row items-start justify-between mb-4">
          ...
          <div class="flex flex-wrap gap-2 mt-2 md:mt-0">
            ...
            <span v-if="proyect.tecnologies.length > 3"
              class="px-3 py-1 text-xs font-medium text-white bg-slate-700 rounded-full">
        ...
        <div class="flex flex-wrap gap-3">
          <a v-if="proyect.links.frontend" :href="proyect.links.frontend" target="_blank"
...
const loadProjects = async () => {
  try {
    projects.value = await getLatestProjects(4);
  } catch (error) {
    projects.value = [];
  } finally {
    loading.value = false;
